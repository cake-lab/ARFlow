...
class SphericalHarmonics:
    ...
    def from_array(sh_coefficients: np.ndarray, channel_order="first"):
        ...
        _n_channels = s_0 if channel_order == "first" else s_1
    ...
    def project_sphere_points(self, sphere_points: u3d.PointCloud):
        matrix_sh_basis = self.get_batched_basis_at(sphere_points.positions)

        c = np.einsum("ij,ik->ijk", matrix_sh_basis, sphere_points.features).sum(axis=0)
    ...
    def reconstruct_to_canvas(self, canvas: u3d.Canvas = None) -> u3d.Canvas:
        if canvas is None:
            canvas = u3d.canvas_equirectangular_panorama(height=128)

        canvas_res = u3d.canvas_equirectangular_panorama(height=canvas.data.shape[0])
...
def draw_equirectangular_panorama(sh, canvas_height=128):
    norm_canvas = u3d.canvas_equirectangular_panorama(height=canvas_height)
...
__all__ = ["SphericalHarmonics", "spherical_harmonics", "draw_equirectangular_panorama"]
