...
class Image:
    ...
    @staticmethod
    def from_base64(
        b64_string: str, dims: Tuple, channels: int, dtype=np.float32
    ) -> Image:
        data = np.frombuffer(
            base64.decodebytes(b64_string.encode("utf-8")), dtype=dtype
        )
    ...
    def normalize(self) -> Image:
        return Image(
            np.array(self._data / np.iinfo(self._data.dtype).max, dtype=np.float32)
        )
