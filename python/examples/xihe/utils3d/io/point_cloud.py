...
class PointCloud:
    ...
    def __array__(self):
        return np.concatenate((self.positions, self.features), axis=-1).astype(
            np.float32
        )
    ...
    def translate(self, vector: Vector3) -> PointCloud:
        return PointCloud(
            np.array(self.positions - vector, dtype=np.float32), self.features
        )
...
def point_cloud_from_rgbd_fov(
    rgb: Image, depth: Image, fov: Vector2, center: Vector2 = None
) -> PointCloud:
    if center is None:
        center = Vector2((rgb.size.x // 2, rgb.size.y // 2))

    _dcx = (center.x - rgb.size.x / 2) / rgb.size.x
    _dcy = (center.y - rgb.size.x / 2) / rgb.size.y

    x = np.repeat(np.arange(rgb.size.x)[np.newaxis, :], rgb.size.y, axis=0)
    x = x / np.max(x) * 2 - 1
    # x = x + dcx

    y = np.repeat(np.arange(rgb.size.y)[:, np.newaxis], rgb.size.x, axis=-1)
    y = y / np.max(y) * 2 - 1
    # y = y + dcy
...
__all__ = [
    "PointCloud",
    "RGBColoredPointCloud",
    "point_cloud_util_split",
    "point_cloud_from_rgbd_fov",
    "point_cloud_from_rgbd_intrinsics",
    "point_cloud",
]
